...
pub struct TimestampedDataCollector<T>
where
    T: Send + Sync + Clone + 'static,
{
...
where
    T: Send + Sync + Clone + 'static,
{
    data: Arc<std::sync::Mutex<TimestampedDataCollector<T>>>,
}
...
where
    T: Send + Sync + Clone + 'static,
{
    pub fn data_mut(&mut self) -> &mut Arc<std::sync::Mutex<TimestampedDataCollector<T>>> {
        &mut self.data
    }
}

impl<T> TimestampedDataCollector<T>
where
    T: Send + Sync + Clone + 'static,
{
    ...
    pub fn spawn(
    ...
    ) -> TimestampedDataHandler<T> {
        {
            ...
            let end = Instant::now();
            if end - start > Duration::new(0, ((1.0 / &hz) / 2.0 * 1e9) as u32) {
                warn!(
                    "Test sample took longer than half the period specified (1/hz). Please consider using another machine or reducing the sampling rate (hz)."
                );
            }
        }
...
mod tests {
    ...
    fn test_runs() {
        ...
        let mut handler = TimestampedDataCollector::spawn(
            60.0,
            Arc::new(move |t| {
