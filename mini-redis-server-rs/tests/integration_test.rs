...
const REQUEST_STORE_SIZE: usize = 32;
...
fn progressive_stress_test() {
    ...
    let (request_sent_tx, mut request_sent_rx) = tokio::sync::mpsc::channel::<()>(1024);
    ...
    rt.block_on(async move {
        let futures =  (0..NUM_CONNECTIONS).map(async |_|{
            let commands_rx_task = send_rate_rx.clone();
            ...
            // Request sender task
            let task_requests_sent_count_tx = request_sent_tx.clone();
            ...
            tokio::spawn(async move {
                ...
                let mut commands_rx = commands_rx_task;
        ...
        let receivers: Vec<OwnedReadHalf> = futures::future::join_all(futures).await;
        tokio::spawn(async move {
            let window_fulfilled_request_count = window_response_count_task;
            let total_fulfilled_request_count = total_response_count_task;
            let mut receivers: Vec<BufReader<OwnedReadHalf>> =
                receivers.into_iter().map(|r| BufReader::new(r)).collect();

            loop {
                let mut any = futures::stream::futures_unordered::FuturesUnordered::new();
                for future in receivers
                    .iter_mut()
                    .map(async |mut r| {
                        read_and_dump_result(&mut r).await;
    ...
    // Evaluate performance and stopping condition (Orchestrator)
    std::thread::spawn(move || {
        loop {
            let fulfilled = *window_response_count.blocking_read();
            ...
            *window_response_count.blocking_write() = 0;
